...
# -----------------------------------------------------------------------------
# IMPORTANT NOTE!
# To do this analysis correctly, at this point we should account for the fact
# that the indices for the SOI and PWC are inversely correlated with ENSO
# indices (e.g., Nino 3.4) simply by convention. This is not a 'real' distinction
# in a climatic sense, and would lead to artificial dissimiilarity between
# otherwise similar reconstructions of the PWC versis ENSO. However, we will 
# proceed with the reconstructions all in their native orientations, to see
# if that is the source of Freund et al.'s findings wrt the PWC being, apparently,
# climatically distinct from ENSO through the past millennium. 
# -----------------------------------------------------------------------------
...
ggplot() +
  ...
  labs(title = "Dendrogram summarising hierarchical relationships between different reconstructions of the\nEl Niño Southern Oscillation or Pacific Walker Circulation",
       subtitle = paste0("Calculated across ", min_year, "–", max_year, " CE; PWC and ENSO reconstructions <i>not</i> aligned")) +
  coord_flip(clip = "off") +
  scale_y_reverse() +
...
ggsave(paste0("tropicalPacific_reconstructions_", min_year, "–", max_year, "hclust_dendrogram_includingLiu2024_reconsNotAligned.png"),
